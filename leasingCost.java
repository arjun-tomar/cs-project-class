public class LeasingCost {
    ...
    {    
        String fuelType; String i; String name;
        int length; int mileage;
        double chargerCost; double monthlyFee; double signingCost; double usage;
        Fuel fuel;

        i = description.substring(description.indexOf("type:")+5);
        fuelType = i.substring(0, i.indexOf(";"));
        i = description.substring(description.indexOf("name:")+5);
        name = i.substring(0, i.indexOf(";"));
        i = description.substring(description.indexOf("due:")+4);
        signingCost = Double.parseDouble(i.substring(0, i.indexOf(";")));
        i = description.substring(description.indexOf("length:")+7);
        length = Integer.parseInt(i.substring(0, i.indexOf(";"))); 
        i = description.substring(description.indexOf("monthly:")+8);
        monthlyFee = Double.parseDouble(i.substring(0, i.indexOf(";")));
        i = description.substring(description.indexOf("mile/unit:")+10);
        usage = Double.parseDouble(i.substring(0, i.indexOf(";")));
        i = description.substring(description.indexOf("allowance")+10);
        mileage = Integer.parseInt(i.substring(0, i.indexOf(";"))); 

        if (fuelType.equals("gas") ){
            fuel = new Fuel(usage);
        }
        else {
            i = description.substring(description.indexOf("charger")+8);
            chargerCost = Double.parseDouble(i.substring(0, i.indexOf(";")));
            fuel = new Fuel(usage, chargerCost);
        }
        Lease lease = new Lease(signingCost, length, monthlyFee, mileage);
    ...
    {
        int fuelType;
        double co2; double fuelCost; double leasingFee; double totalCost;

        for(int i = 0; i< vehicles.length; i++)
        {
            fuelType = vehicles[i].getFuel().getType();
            
            if (fuelType == 1)
            {
                co2 = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION);
                ...
                leasingFee = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost());
                totalCost = fuelCost + leasingFee;
                vehicles[i].setTotalCost(totalCost);
            }

            else if(fuelType != 1)
            {
                co2 = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION);
                ...
                leasingFee = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost()); 
                totalCost = fuelCost + leasingFee + vehicles[i].getFuel().getCharger();
